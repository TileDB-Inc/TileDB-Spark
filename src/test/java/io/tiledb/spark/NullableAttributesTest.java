...
public class NullableAttributesTest extends SharedJavaSparkSession {
  ...
  public Dataset<Row> createDenseDataset(SparkSession ss) {
    ...
    rows.add(RowFactory.create(1, 2, 3f, 4));
    rows.add(RowFactory.create(2, 1, 4f, null));
  ...
  /**
   * Dense dataset with var-size attributes.
   *
   * @param ss The spark Session
   * @return The created dataframe.
   */
  public Dataset<Row> createDenseDatasetVar(SparkSession ss) {
    StructField[] structFields =
        new StructField[] {
          new StructField("rows", DataTypes.IntegerType, false, Metadata.empty()),
    ...
    List<Row> rows = new ArrayList<>();
    rows.add(RowFactory.create(1, "aab"));
    rows.add(RowFactory.create(2, null));
    rows.add(RowFactory.create(3, "cc"));
    rows.add(RowFactory.create(4, null));
    rows.add(RowFactory.create(5, "dee"));
    rows.add(RowFactory.create(6, "ffg"));
    rows.add(RowFactory.create(7, null));
    rows.add(RowFactory.create(8, "hh"));
    StructType structType = new StructType(structFields);
    Dataset<Row> df = ss.createDataFrame(rows, structType);
    return df;
  }
  ...
  @Test
  public void denseWriteTest() throws Exception {
  ...
  @Test
  public void denseWriteVarAttTest() throws Exception {
    Dataset<Row> dfReadFirst = createDenseDatasetVar(session());
    dfReadFirst
        .write()
        .format("io.tiledb.spark")
        .option("uri", writeArrayURI)
    ...
    Dataset<Row> dfRead =
        session().read().format("io.tiledb.spark").option("uri", writeArrayURI).load();
    Assert.assertEquals(dfReadFirst.count(), dfRead.count());
    dfRead.createOrReplaceTempView("tmp");
    List<Row> rows = session().sql("SELECT * FROM tmp").collectAsList();
    Assert.assertEquals(8, rows.size());
